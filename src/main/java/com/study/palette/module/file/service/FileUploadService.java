...
public class FileUploadService {
    ...
    public FileUploadDto saveFile(MultipartFile multipartFile) throws IOException {
        String uploadFilename = getRandomString(Objects.requireNonNull(multipartFile.getOriginalFilename()));
        String type = multipartFile.getContentType();
        ObjectMetadata metadata = new ObjectMetadata();
        metadata.setContentLength(multipartFile.getSize());
        metadata.setContentType(multipartFile.getContentType());

        amazonS3.putObject(bucket, pathPrefix + "/" + uploadFilename, multipartFile.getInputStream(), metadata);
        ;
